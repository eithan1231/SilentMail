...
{
  if(strtolower(substr($p, 0, 4)) == "cid:") {
    // Inline attachment
    global $route;
    return $route->getRoutePath('inboxInlineAsset', [
      'inbox_id' => $GLOBALS['S']['mailbox_item']['inbox_id'],
      'content_id' => substr($p, 4)
    ]);
  }
    ...
    if (((isset($d['*']) ? isset($d[$k]) : !isset($d[$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or (isset($aNO[$k]) && !isset($d['on*'])) or (isset($aNA[$k]) && !isset($d['aria*'])) or (!isset($d['data*']) && preg_match('`data-((?!xml)[^:]+$)`', $k))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
      ...
      if ($k == 'style' && !$C['style_pass']) {
        ...
        $v = str_replace("­", ' ', (strpos($v, '&') !== false ? str_replace(array(
          ...
          '&#173;',
          '&shy;'
        ), ' ', $v) : $v)); // double-quoted char: soft-hyphen; appears here as "­" or hyphen or something else depending on viewing software
        if ($k == 'srcset') {
          $v2 = '';
        ...
        if ($k == 'href') { // X-spam
          ...
          if (isset($GLOBALS['S']['mailbox_item'])) {
            // $GLOBALS['S']['mailbox_item']

            if($e == 'a') {
              // A element, redirect through a redirect-route.
              global $route;
              $v = $route->getRoutePath('urlRedirect', [
                'url' => $v
              ]);
            }
          }
